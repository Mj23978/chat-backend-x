...
func (t *Tracer) Setup() error {
	...
	case "jaeger":
		...
		logger.Infof("Jaeger tracer configured!")
	case "zipkin":
		...
		t.closer = reporter
		t.tracer = opentracing.GlobalTracer()
		logger.Infof("Zipkin tracer configured!")
	case "datadog":
		...
		t.tracer = opentracing.GlobalTracer()
		logger.Infof("DataDog tracer configured!")
	case "":
		logger.Infof("No tracer configured - skipping tracing setup")
	default:
