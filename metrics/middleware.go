...
import (
	...
	//cmd "github.com/mj23978/chat-backend-x/cobra"
...
func New(
	...
	if err != nil {
		logger.Fatalf("Unable to initialise software quality assurance features : %w", err)
		return nil
	}
	...
	if err != nil {
		...
		if optOut {
			logger.Warnf(`Command line argument "--disable-telemetry" has been deprecated and will be removed in an upcoming release. Use "--sqa-opt-out" instead.`)
		}
		//cmd.Must(err, `Unable to get command line flag "sqa-opt-out" and "disable-telemetry": %s`, err)
	}
	...
	if !optOut {
		...
		if optOut {
			logger.Warnf(`Environment variable "DISABLE_TELEMETRY" has been deprecated and will be removed in an upcoming release. Use configuration key "sqa.opt_out: true" or environment variable "SQA_OPT_OUT=true" instead.`)
		}
	}

	if !optOut {
		logger.Infof("Software quality assurance features are enabled. Learn more at: https://www.ory.sh/docs/ecosystem/sqa")
		oi = analytics.OSInfo{
	...
	m := &Service{
		...
		context: &analytics.Context{
			...
			UserAgent: "github.com/mj23978/chat-backend-x/metrics.Service/v0.0.1",
		},
	}
...
//		})
//	}); err != nil {
//		logger.Debugf("Could not commit anonymized environment information: %w", err)
//	}
//}
...
func (sw *Service) ObserveMemory() {
	...
	for {
		...
		if err := sw.c.Enqueue(analytics.Track{
			...
			logger.Debugf("Could not commit anonymized telemetry data: %w", err)
		}
