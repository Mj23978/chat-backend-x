...
var ErrNoneFunction = errors.New("Kind of Value for listener is not Channel.")
...
func (emitter *Emitter) AddListener(event, listener interface{}) *Emitter {
	...
	cn := reflect.ValueOf(listener)

	fmt.Printf("pointer of cn : %v - pointer of listner : %v", cn, listener)

	if reflect.Chan != cn.Kind() {
	...
	emitter.events[event] = append(emitter.events[event], cn)
...
func (emitter *Emitter) RemoveListener(event, listener interface{}) *Emitter {
	...
	if events, ok := emitter.events[event]; ok {
		...
		for _, listener := range events {
			if cn.Pointer() != listener.Pointer() {
...
func (emitter *Emitter) Once(event, listener interface{}) *Emitter {
	...
	emitter.Lock()
	emitter.onces[cn] = reflect.ValueOf(run)
	emitter.Unlock()
...
func (emitter *Emitter) Emit(event interface{}, arguments ...interface{}) *Emitter {
	...
	for _, cn := range listeners {
		go func(cn reflect.Value) {
			defer wg.Done()
			...
			if nil != emitter.recoverer {
				defer func() {
					if r := recover(); nil != r {
						err := fmt.Errorf("%v", r)
						emitter.recoverer(event, cn.Interface(), err)
					}
				}()
			}

			for i := 0; i < len(arguments); i++ {
				cn.TrySend(reflect.ValueOf(arguments[i]))
			}
		}(cn)
	}
